...
from pipeline_config import START_TIME, DURATION, ASD_DEBUG, PADDING
...
def asd_stage():
    ...
    print(f"Found {total_json} JSON files in {TRIM_OUTPUT_DIR}")
    ...
    for json_file in json_files:
        ...
        for i, segment in enumerate(segments):
            ...
            print(f"Processing: {segment_path} ({i + 1}/{segments_length})")
            ...
            #print(tracks)
            #manifest = generate_manifest(tracks, scores, args, segment["start"])
            #print(manifest)
            ...
            print(f"Most confident track: {best_index}, score: {best_score:.3f}")
...
def _generate_manifest(faces, word_timestamps, start_time, segment_start, fps=25.0, threshold=0.5, min_duration=0.2):
    ...
    print(start_time, segment_start)
    return manifest
...
def run_asd_stage():
    ...
    print(f"ASD stage finished in {elapsed_time}\n")
