import sys, time, os, tqdm, torch, glob, subprocess, warnings, cv2, pickle, numpy, math, python_speech_features, contextlib
...
from pipeline_config import (
    ASD_TALKNET_MODEL_PATH, ASD_YOLO_MODEL_PATH, ASD_MAX_WORKERS_DATALOADER,
    ASD_FACEDET_SCALE, ASD_MIN_TRACK, ASD_NUM_FAILED_DET, ASD_MIN_FACE_SIZE,
    ASD_CROP_SCALE, ASD_DURATION_SET
)
...
pretrained_model_path = ASD_TALKNET_MODEL_PATH
yolo_model_path = ASD_YOLO_MODEL_PATH
data_loader_thread = ASD_MAX_WORKERS_DATALOADER
face_detection_scale = ASD_FACEDET_SCALE
min_track = ASD_MIN_TRACK # Number of min frames for each shot
num_failed_det = ASD_NUM_FAILED_DET # Number of missed detections allowed before tracking is stopped
min_face_size = ASD_MIN_FACE_SIZE # Minimum face size in pixels
crop_scale = ASD_CROP_SCALE # Scale bounding box
...
def scene_detect(video_path, pyworkPath, save = False, start_frame = 0, end_frame = None):
...
def initialize_detector(device='cuda'):
    model = YOLO(yolo_model_path)  # You may need to download a YOLOv8 face detection model
    model.to(device)
    return model

def predict_faces(DET, pyframesPath, pyworkPath, start_frame=0, batch_size=64):
    flist = sorted(glob.glob(os.path.join(pyframesPath, '*.jpg')))
    images = [cv2.imread(fname) for fname in flist]

    return predict_faces_from_frames(DET, images, pyworkPath, start_frame=start_frame, batch_size=batch_size)

def predict_faces_from_frames(DET, frames, pyworkPath, start_frame=0, batch_size=64):
    dets = []
...
def crop_video(track, cropFile, pyframesPath, audioFilePath, start_frame = 0):
    # CPU: crop the face clips
    # flist = glob.glob(os.path.join(pyframesPath, '*.jpg')) # Read the frames
    # flist.sort()

    frame_files = sorted(glob.glob(os.path.join(pyframesPath, '*.jpg')))
    frame_cache = [cv2.imread(f) for f in frame_files]
    ...
    avg_s = numpy.mean(dets['s']) * (1 + crop_scale)
    target_size = round(2 * avg_s)
    vOut = cv2.VideoWriter(cropFile + 't.avi', cv2.VideoWriter_fourcc(*'XVID'), 25, (target_size, target_size))
    ...
    for fidx, frame in enumerate(track['frame']):
        ...
        vOut.write(cv2.resize(face, (target_size, target_size), interpolation=cv2.INTER_LANCZOS4))
...
def crop_video_from_frames(track, cropFile, frames, audioFilePath, start_frame = 0):
    # CPU: crop the face clips

    # FIXME: similar to crop_video
    #vOut = cv2.VideoWriter(cropFile + 't.avi', cv2.VideoWriter_fourcc(*'XVID'), 25, (224,224))# Write video
    dets = {'x':[], 'y':[], 's':[]}
    for det in track['bbox']: # Read the tracks
        dets['s'].append(max((det[3]-det[1]), (det[2]-det[0]))/2) 
...
def evaluate_network(s, files, pycropPath):
...
def visualization(tracks, scores, pyframesPath, pyaviPath):
...
def visualization_from_frames(tracks, scores, frames, pyaviPath):
...
def main(
    ...
    if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
        ...
        command = ("ffmpeg -y -i '%s' -q:v 1 -vf fps=25 -ss %s -t %s -threads %d -f image2 %s -loglevel panic" % \
            (video_path, start, duration, data_loader_thread, os.path.join(pyframesPath, '%06d.jpg')))
        subprocess.call(command, shell=True, stdout=None)
    ...
    with suppress_stderr():
        scene = scene_detect(video_path, pyworkPath, save = True, start_frame = start_frame, end_frame = int(end_seconds * fps))
    ...
    if face_boxes:
        ...
        if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
            faces = predict_faces_from_frames(DET, frames, pyworkPath)
        else:
            faces = predict_faces(DET, pyframesPath, pyworkPath)
    ...
    for ii, track in enumerate(allTracks):
        if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
            vidTracks.append(crop_video_from_frames(track, os.path.join(pycropPath, str(ii)), frames, audioFilePath))
        else:
            vidTracks.append(crop_video(track, os.path.join(pycropPath, str(ii)), pyframesPath, audioFilePath))
    ...
    files.sort()
    scores = evaluate_network(s, files, pycropPath)
    ...
    if return_visualization:
        ...
        if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
            visualization_from_frames(vidTracks, scores, frames, pyaviPath)
        else:
            visualization(vidTracks, scores, pyframesPath, pyaviPath)
    ...
    return vidTracks, scores, files, faces