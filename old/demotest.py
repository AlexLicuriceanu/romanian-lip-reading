...
def main(args):
    ...
    drawing_mode = args.drawing_mode
    ...
    # Get video writer if saving
    out_writer = None
    if args.save_output:
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out_writer = cv2.VideoWriter('output.mp4', fourcc, cap.get(cv2.CAP_PROP_FPS), (cap_width, cap_height))
    ...
    while cap.isOpened():
        ...
        if background_mode == 'on':
            ...
            canvas = np.zeros_like(resized_frame, dtype=np.uint8)
            ...
            for face, score in zip(final_landmarks, scores):
                if score > 0.95:
                    for keypoint in face:
                        ...
                        y = keypoint[1]
                        z = keypoint[2]
                        if drawing_mode in ['partial','point']:
                            cv2.circle(
                                img=canvas,
                                center=(x, y),
                                radius=2,
                    ...
                    for start_point_idx, end_point_idx in connection_points:
                        cv2.line(
                            img=canvas,
        ...
        if key == 27: # ESC
            ...
            background_mode = 'off' if background_mode == 'on' else 'on'
        elif key == 99: # C
            mode_idx = DRAWING_MODES.index(drawing_mode)
...
if __name__ == "__main__":
    ...
    "--save_output",
    action="store_true",
    help='Save the output to output.mp4'
    )
